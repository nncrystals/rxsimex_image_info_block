...
public:
  explicit image_info_block(SimStruct *s) : rx_remote_block(s) {
    ...
    param_num_input = register_scalar_parameter<int>("max_num_input", false);
    param_num_output = register_scalar_parameter<int>("max_num_output", false);
  }
...
public:
  void on_parameter_updated() override {
    ...
    set_remote_accessible_port(register_input_port<double>(
        "inputs", {param_num_input->data}, false, false, false));

    set_remote_accessible_port(register_output_port<double>(
        "outputs", {param_num_output->data}, false, false));
  }
  void on_start() override {
    rx_remote_block::on_start();
    session_disconnected.get_observable().subscribe(
        subscriptions, [&](auto next) {
          if (sessions.size() == 0) {
            auto worker = rl->get_scheduler().create_worker(subscriptions);
            auto stopper = rxcpp::schedulers::make_schedulable(
                worker, [](rxcpp::schedulers::schedulable self) {
